...
const MainApp = () => {
  ...
  const submitIncident = async () => {
    if (!incidentFormData.title || !incidentFormData.description || !incidentFormData.location) {
    ...
    try {
      let coordinates = incidentFormData.coordinates;
      
      // If no GPS coordinates, try to geocode the address
      if (!coordinates && incidentFormData.location) {
        console.log('🗺️ Versuche Adresse zu geocoden:', incidentFormData.location);
        coordinates = await geocodeAddress(incidentFormData.location);
        
        if (coordinates) {
          Alert.alert('✅ Adresse gefunden', `Koordinaten ermittelt:\n${coordinates.display_name}`);
        } else {
          Alert.alert('⚠️ Warnung', 'Adresse konnte nicht auf der Karte lokalisiert werden. Vorfall wird trotzdem gemeldet.');
        }
      }
      ...
      const incidentData = {
        ...
        coordinates: coordinates, // Include coordinates for map display
      ...
      Alert.alert('✅ Erfolg', 'Vorfall wurde erfolgreich gemeldet!');
